...
function icon(name) {
  return `<svg class="icon" aria-hidden="true"><use href="/static/icons.svg?v=1.3.9#${name}"></use></svg>`;
}
...
const PLACEHOLDER_COPY = {
  settings: ["设置（开发中）", "设置视图将在此展开：租户配置、成员与集成。"],
};
...
function showAppView(name) {
  ...
  if (els.placeholderView) {
    ...
    if (els.placeholderView.hidden === false) {
      const [title, text] = PLACEHOLDER_COPY[name] || PLACEHOLDER_COPY.settings;
      if (els.placeholderTitle) els.placeholderTitle.textContent = title;
      if (els.placeholderText) els.placeholderText.textContent = text;
    }
  }
}
