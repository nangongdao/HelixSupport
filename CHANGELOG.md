...
- **测试覆盖率提升**（2026-09-03）：
  ...
  - 第六轮：新增 `tests/test_outbox_consumer.py`（6 例）完整覆盖 `app/outbox_consumer.py` outbox 事件消费者模块（发布到 webhook、未映射事件无端点、无活跃订阅者、多事件处理、待处理计数、webhook 类型映射注册），`outbox_consumer.py` 模块覆盖率从 **0.00% → 100.00%**（30 stmts 全覆盖）。总体覆盖率持续提升中，90% 目标需额外覆盖 ~323 行，留待后续迭代。
